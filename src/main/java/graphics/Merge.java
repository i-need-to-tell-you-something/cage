...
public class Merge {
	...
	public static JFrame Joonista(Kass kiisu) throws IOException {
		...
		// seda vaja)
		BufferedImage varv1 = ImageIO.read(new File(FilePaths.gfx+"/Blank.png"));
		BufferedImage varv2 = ImageIO.read(new File(FilePaths.gfx+"/Blank.png"));
		BufferedImage varv3 = ImageIO.read(new File(FilePaths.gfx+"/Blank.png"));// siiami
																				// jne
																				// kassidele

		// Loon kaks triipude pilti (Kaks, sest kilpkonnakassiga mustriga läheb
		// seda vaja)
		BufferedImage triibud1 = ImageIO.read(new File(FilePaths.gfx+"/Blank.png"));
		BufferedImage triibud2 = ImageIO.read(new File(FilePaths.gfx+"/Blank.png"));
		...
		if (Maarang.cLookus(fenoahel) != 7) {
			...
			if (Maarang.wlookus(fenoahel) != 0) {
				// On valgeks värvunud
				varv1 = Filter.tootle(FilePaths.gfx+"/Hall.png", Varvid.hue[12],
				...
				if (Maarang.kasKilpkonn(fenoahel)) {
					...
					varv1 = Filter.tootle(FilePaths.gfx+"/Hall-1.png",
							Varvid.hue[x[0]], Varvid.saturation[x[0]],
							Varvid.lumbon[x[0]]);
					varv2 = Filter.tootle(FilePaths.gfx+"/Hall-2.png",
					...
					varv1 = Filter.tootle(FilePaths.gfx+"/Hall.png", Varvid.hue[x],
		...
		if (Maarang.kasPikaKarv(fenoahel)) {
			// Kui pikakarvaline
			karv = ImageIO.read(new File(FilePaths.gfx+"/Pikakarvaline.png"));
			taust = ImageIO.read(new File(FilePaths.gfx+"/Pikk_taust.png"));
		}

		else {
			// Kui lühikarvaline
			karv = ImageIO.read(new File(FilePaths.gfx+"/Lyhikarvaline.png"));
			taust = ImageIO.read(new File(FilePaths.gfx+"/Lyhike_taust.png"));
		}
